...
Each keyboard plays the same patch through independently owned voices. Detune retunes its held notes; octave changes apply to new presses and update key labels. Each keyboard’s **Sustain latch** holds released notes until switched off; replaying a latched note retriggers it. Keys still physically held keep sounding when the latch turns off. Switching tabs or applications preserves latched notes. Ordinary browser keys release on focus loss, while MIDI remains responsive in the background. Stop all clears both latches. Latches are performance state and are not saved with patches. Computer keys play keyboard 1. A shared 16-voice limit applies across both keyboards and MIDI.
